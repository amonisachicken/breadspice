...
- ✅ 元件库：电阻、电容、电位器、二极管 ×2（1N4148 / 1N5817）、LED ×3（红/绿/蓝）、
...
- ✅ 双击属性：数值 + 单位、正弦参数（频率/交流/直流/相位）、电位器（总阻值 + 百分比滑块）、
  导线颜色、仪表读数、示波器、半导体/IC 引脚说明
- ✅ 保存布局（`.bread`）/ 撤销 / 重做 / 下载 / 导入 / 预览模式；仿真选项随布局持久化（不进撤销/重做栈）
...
| 电位器 | 两个串联电阻 `R<name>A`（引脚 1-2，R1）与 `R<name>B`（引脚 2-3，R2），`R1 = 总阻值 × 百分比`，两端到底时用 0.001Ω 兜底 |
